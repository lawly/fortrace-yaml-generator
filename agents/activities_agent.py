...
class ActivitiesAgent(AgentBase):
    ...
    def __extract_response(self, response: str):
        response = response.split(f'<activities>')[1].split(f'</activities>')[0]
        return response
    ...
    def node_action(self, state: Any) -> RunnableLike:
        ...
        logging.info(f"ActivitiesAgent response: {response.content}")
        activities: str = self.__extract_response(response.content)
        logging.info(f"Extracted activities: {activities}")
